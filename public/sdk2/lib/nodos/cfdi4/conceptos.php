...
function mf_nodo_conceptos(array &$datosTodos)
{
    ...
    foreach ($datos as $idx => $concepto)
    {
        ...
        if(isset($concepto['ACuentaTerceros']))
        {
            $ACuentaTerceros = $concepto['ACuentaTerceros'];
            $atrsACuentaTerceros = mf_atributos_nodo($ACuentaTerceros, 'conceptos.ACuentaTerceros');
            $nodoAcuentaTerceros .= "<cfdi:ACuentaTerceros $atrsACuentaTerceros />";
        }
        ...
        $sub = "<cfdi:Concepto $subatrs>$impuestos$infoaduanera$predial$nodoComplemento$nodoparte$nodoAcuentaTerceros</cfdi:Concepto>";
        $subs .= $sub;
    }
