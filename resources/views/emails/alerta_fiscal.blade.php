<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>

</head>
<body>

<div style="margin: 0 !important; padding: 0 !important; width: 100%; max-width: 816px;">
    <!-- Imagen superior -->
    <img src="https://apicontafacil.internow.com.mx/images_uploads/logos/logo_base.png" alt="Imagen Superior"
        style="display: block;
            margin: 0 auto;
            width: 150px;
            height: auto;" 
    >

    <div style="background-color: #FFFFFF;
        padding: 20px;
        border: 20px solid #4285CB;">

      <div style="text-align: center;">
          Emitido por Conta Fácil AM
      </div>
      <br><br><br>

        <p>Hola {{ $usuario->nombre }},</p>

        <p>Estos son los eventos fiscales relevantes para ti hoy ({{ \Carbon\Carbon::today()->format('d/m/Y') }}):</p>

        <ul>
        @foreach($eventos as $evento)
            <li><strong>{{ $evento->titulo }}</strong> (Tipo: {{ $evento->tipo }}) - {{ $evento->descripcion }}</li>
        @endforeach
        </ul>

        <p>Por favor toma las acciones necesarias.</p>

      <br><br><br>
      <div style="text-align: center;">
        <a href="https://wa.me/5217751936000" target="_blank" style="color: #000000;">
          Internow IA
        </a>
      </div>

    </div>

</div>

</body>
</html>